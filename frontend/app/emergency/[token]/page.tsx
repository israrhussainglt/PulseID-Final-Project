...
export default async function EmergencyPage({ params }: { params: { token: string } }) {
  const result = await serverFetch<EmergencyResponse>(`/api/emergency/${params.token}`);
  ...
  return (
    <main className="min-h-screen bg-ink text-white">
      <div className="max-w-lg mx-auto px-6 py-10">
        ...
        <p className="text-sm text-white/60 mb-1">Scanned emergency ID for</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <Card className="!bg-white/5 !border-white/10 p-4">
            <div className="eyebrow text-alert mb-1">Emergency access</div>
            <div className="font-mono text-sm text-white mt-2">Single-use QR</div>
          </Card>
        </div>
        ...
        <p className="text-xs text-white/40 leading-relaxed">
          This is a scoped emergency view — only life-critical information is shown. This access has
          been logged and {patient.fullName.split(" ")[0]} can see it in their audit log. For
          security, the QR code that brought you here has now been rotated and cannot be scanned
          again.
        </p>
