...
export function PatientSearch() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(apiUrl(`/api/patients/search?q=${encodeURIComponent(query.trim())}`), {
          credentials: "include",
        });
        ...
        <Card className="absolute z-10 mt-2 w-full overflow-hidden">
          ...
          {!loading &&
            results.map((r) => (
              <button
                key={r.id}
                onClick={() => router.push(`/doctor/patients/${r.id}`)}
                className="w-full text-left px-4 py-3 hover:bg-teal-light/60 transition-colors border-b border-line last:border-b-0 flex items-center justify-between"
              >
                <div>
